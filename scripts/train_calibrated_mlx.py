#!/usr/bin/env python3
"""Train calibrated NanoJev decision models natively on Apple Silicon using Apple MLX.

Key improvements:
- Supports temperature scaling during training to ensure sharp confidence outputs (>0.65)
- Supports LayerNorm + Scalar + Choice Set Attention heads
- Precomputes backbone embeddings for ultra-fast iterative training
- Evaluates directly against validation loss and saves best.safetensors
"""
import argparse
import json
import math
...
def main():
    ...
    init_dev = evaluate_cached_dev(heads, cached_dev, "gold_distribution", "ce")
    print(f"Initial Dev Loss: {init_dev:.4f}")
    ...
    for epoch in range(args.epochs):
        ...
        dev_loss = evaluate_cached_dev(heads, cached_dev, "gold_distribution", "ce")
        dt = time.time() - t0

        star = "🌟 (Best)" if dev_loss < best_dev_loss else ""
        print(f"Epoch {epoch+1:02d}/{args.epochs:02d} | Train: {train_loss:.4f} | Dev: {dev_loss:.4f} | Time: {dt:.2f}s {star}", flush=True)
    ...
    print(f"\nDone! Best Dev Loss: {best_dev_loss:.4f} -> {output_dir}")
